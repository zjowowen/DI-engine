...
from ding.framework.middleware import CkptSaver, \
    ...
    OffPolicyLearner, final_ctx_saver, nstep_reward_enhancer, eps_greedy_handler, offline_data_fetcher, trainer, offline_logger, \
...
from ding.config.CSQLDiscrete import supported_env_cfg
from ding.config.CSQLDiscrete import supported_env


class CSQLDiscreteAgent:
    ...
    def __init__(
        ...
        if env_id is not None:
            assert env_id in CSQLDiscreteAgent.supported_env_list, "Please use supported envs: {}".format(
                CSQLDiscreteAgent.supported_env_list
            )
            if cfg is None:
            ...
            assert cfg.env.env_id in CSQLDiscreteAgent.supported_env_list, "Please use supported envs: {}".format(
                CSQLDiscreteAgent.supported_env_list
            )
        default_policy_config = EasyDict({"policy": CSQLDiscretePolicy.default_config()})
        default_policy_config.update(cfg)
        cfg = default_policy_config

        if exp_name is not None:
            cfg.exp_name = exp_name
        self.cfg = compile_config(cfg, policy=CSQLDiscretePolicy)
        ...
        if model is None:
            model = QRDQN(**self.cfg.policy.model)
        self.buffer_ = DequeBuffer(size=self.cfg.policy.other.replay_buffer.replay_buffer_size)
        self.policy = CSQLDiscretePolicy(self.cfg.policy, model=model)
    ...
    def train(
        ...
        with task.start(ctx=OfflineRLContext()):
            ...
            task.use(offline_data_fetcher(self.cfg, self.dataset))
    ...
    def deploy(
        ...
        for seed in seeds:
            ...
            while True:
                ...
                if done:
                    break
            logging.info(f'CSQLDiscrete deploy is finished, final episode return with {step} steps is: {return_}')
    ...
    def collect_data(
        ...
        with task.start(ctx=OfflineRLContext()):
            ...
            task.run(max_step=1)
        logging.info(
            f'CSQLDiscrete collecting is finished, more than {n_sample} samples are collected and saved in `{save_data_path}`'
        )

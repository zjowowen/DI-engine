...
from ding.config.CQLDiscrete import supported_env_cfg
from ding.config.CQLDiscrete import supported_env


class CQLDiscreteAgent:
    ...
    def __init__(
        ...
        if env_id is not None:
            assert env_id in CQLDiscreteAgent.supported_env_list, "Please use supported envs: {}".format(
                CQLDiscreteAgent.supported_env_list
            )
            if cfg is None:
            ...
            assert cfg.env.env_id in CQLDiscreteAgent.supported_env_list, "Please use supported envs: {}".format(
                CQLDiscreteAgent.supported_env_list
            )
        default_policy_config = EasyDict({"policy": CQLDiscretePolicy.default_config()})
        default_policy_config.update(cfg)
        cfg = default_policy_config

        if exp_name is not None:
            cfg.exp_name = exp_name
        self.cfg = compile_config(cfg, policy=CQLDiscretePolicy)
        ...
        self.policy = CQLDiscretePolicy(self.cfg.policy, model=model)
    ...
    def deploy(
        ...
        for seed in seeds:
            ...
            while True:
                ...
                if done:
                    break
            logging.info(f'CQLDiscrete deploy is finished, final episode return with {step} steps is: {return_}')
    ...
    def collect_data(
        ...
        with task.start(ctx=OfflineRLContext()):
            ...
            task.run(max_step=1)
        logging.info(
            f'CQLDiscrete collecting is finished, more than {n_sample} samples are collected and saved in `{save_data_path}`'
        )

...
from ding.config.example.IMPALA import supported_env_cfg
from ding.config.example.IMPALA import supported_env
...
class IMPALAAgent:
    ...
    def __init__(
        ...
        if env_id is not None:
            assert env_id in IMPALAAgent.supported_env_list, "Please use supported envs: {}".format(
            ...
            assert cfg.env.env_id in IMPALAAgent.supported_env_list, "Please use supported envs: {}".format(
                IMPALAAgent.supported_env_list
            )
        default_policy_config = EasyDict({"policy": IMPALAPolicy.default_config()})
        ...
        self.buffer_ = DequeBuffer(
            size=self.cfg.policy.other.replay_buffer.replay_buffer_size, sliced=cfg.policy.other.replay_buffer.sliced
        )

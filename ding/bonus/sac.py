...
from ding.model import BaseQAC, QAC
from ding.model import StochasticPolicy, QModel
    ...
    def __init__(
        ...
        if model is None:
            if hasattr(self.cfg.policy.model, "customized_model") and self.cfg.policy.model.customized_model:
                ...
                critic = QModel(self.cfg.policy.model.critic)
                model = BaseQAC(actor=actor, critic=critic, action_space=self.cfg.policy.action_space)
            else:

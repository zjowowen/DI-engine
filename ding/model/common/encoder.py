...
class IMPALACnnResidualBlock_Old(nn.Module):
    """
    Residual basic block (without batchnorm) in IMPALA cnn encoder.
    Preserves channel number and shape
    """
...
class IMPALACnnDownStack_Old(nn.Module):
    """
    Downsampling stack from Impala CNN
    """
    ...
    def __init__(self, in_channnel, nblock, out_channel, scale=1, pool=True, **kwargs):
        ...
        s = scale / math.sqrt(nblock)
        self.blocks = nn.ModuleList([IMPALACnnResidualBlock_Old(out_channel, scale=s, **kwargs) for _ in range(nblock)])
...
class IMPALAConvEncoder_Old(nn.Module):
    name = "IMPALAConvEncoder"  # put it here to preserve pickle compat
    ...
    def __init__(
        ...
        self.stacks = nn.ModuleList()
        for out_channel in channels:
            stack = IMPALACnnDownStack_Old(curshape[0], nblock=nblock, out_channel=out_channel, scale=s, **kwargs)
            self.stacks.append(stack)
            curshape = stack.output_shape(curshape)

...
@POLICY_REGISTRY.register('ppg_mae')
class PPGMAEPolicy(Policy):
    """
    Overview:
    ...
    def _init_learn(self) -> None:
        ...
        """
        # Optimizer
        self._optimizer_encoder_ac = Adam(self._model.actor_critic.parameters(), lr=self._cfg.mae_learn.learning_rate)
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        if self._cfg.mae_learn.disable_mae_reconstruct and data['collect_iter'][0].item(
        ) <= self._cfg.mae_learn.stop_trainning_iter:
        ...
        for epoch in range(self._cfg.learn.actor_epoch_per_collect):
            for policy_data in split_data_generator(data, self._cfg.learn.batch_size, shuffle=True):
                ...
                if 'report_overall_gradient_norm' in dir(self._learn_model.actor_critic.actor[0]):
                    actor_encoder_overall_gradient = self._learn_model.actor_critic.actor[
                        0].report_overall_gradient_norm()
                    ...
                    actor_encoder_overall_gradient_weight = 0.0
                    actor_encoder_overall_gradient_bias = 0.0

                current = self._optimizer_ac.state_dict()['state']
        ...
        for epoch in range(self._cfg.learn.critic_epoch_per_collect):
            for value_data in split_data_generator(data, self._cfg.learn.batch_size, shuffle=True):
                ...
                if "report_overall_gradient_norm" in dir(self._learn_model.actor_critic.critic[0]):
                    critic_encoder_overall_gradient = self._learn_model.actor_critic.critic[
                        0].report_overall_gradient_norm()
                    ...
                    critic_encoder_overall_gradient_weight = 0.0
                    critic_encoder_overall_gradient_bias = 0.0

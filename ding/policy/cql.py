...
from ding.rl_utils import v_1step_td_data, v_1step_td_error, get_train_sample, \
    ...
    q_nstep_td_data, q_nstep_sql_td_error
...
@POLICY_REGISTRY.register('csql_discrete')
class CSQLDiscretePolicy(SQLPolicy):
    """
        Overview:
            Policy class of CQL-SQL algorithm in discrete environments.
    """
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        # 2. compute value loss(negative_sampling - dataset_expec)
        replay_action_one_hot = F.one_hot(data['action'], self._cfg.model.action_shape)
        replay_chosen_q = (q_value * replay_action_one_hot).sum(dim=1)

        dataset_expec = replay_chosen_q.mean()

        negative_sampling = torch.logsumexp(q_value, dim=1).mean()
    ...
    def _monitor_vars_learn(self) -> List[str]:
        return ['cur_lr', 'total_loss', 'q_target', 'q_value', 'record_value_function']

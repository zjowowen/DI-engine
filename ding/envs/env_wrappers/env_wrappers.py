...
def get_cam(img, model):
    ...
    # assert False
    # target_layers = [model.encoder.main[-1]]
    # target_layers = [model.encoder.stacks[0].firstconv]
    # target_layers = [model.encoder.stacks[2].firstconv]
    # new
    target_layers = [model.encoder.stacks[2].blocks[1].conv1]
    # target_layers = [model.actor_encoder.stacks[2].blocks[1].conv1]
    # target_layers = [model.critic_encoder.stacks[2].blocks[1].conv1]
    # old
    # target_layers = [model.actor_encoder.stacks[2].blocks[1].conv1]
    # target_layers = [model.critic_encoder.stacks[2].blocks[1].conv1]
...
def dump_multi_arr2video(video_num, arr, video_folder):
    fps = 6
    # size = (256, 240)
    size = (512, 512)
    fourcc = cv2.VideoWriter_fourcc(*'MP4V')
    ...
    for i in range(video_num):
        out.append(cv2.VideoWriter(video_folder + '/cam_pure-{}.mp4'.format(i), fourcc, fps, size))
        out1.append(cv2.VideoWriter(video_folder + '/obs_pure-{}.mp4'.format(i), fourcc, fps, size))
        out2.append(cv2.VideoWriter(video_folder + '/merged-{}.mp4'.format(i), fourcc, fps, size))
        for frame, obs in arr[i]:
            frame = (255 * frame).astype('uint8').squeeze(0)
            frame_c = cv2.resize(cv2.applyColorMap(frame, cv2.COLORMAP_JET), size)
            out[i].write(frame_c)
            ...
            merged_frame = cv2.addWeighted(obs, 0.6, frame_c, 0.4, 0)
            out2[i].write(merged_frame)

    # assert False


def get_multi_cam(img, model, layer, index=None):
    # print(model.state_dict().keys())
    # assert False
    # target_layers = [model.encoder.main[-1]]
    # target_layers = [model.encoder.stacks[0].firstconv]
...
@ENV_WRAPPER_REGISTRY.register('RecordCAM')
class RecordCAM(gym.Wrapper):
    ...
    def seed(self, seed: int, dynamic_seed: bool = True) -> None:
        self._env.seed(seed, dynamic_seed)


@ENV_WRAPPER_REGISTRY.register('RecordMultiCAM')
class RecordMultiCAM(gym.Wrapper):
    ...
    def __init__(
        ...
        super(RecordMultiCAM, self).__init__(env)
        self._env = env
        self.cam_model = cam_model
    ...
    def reset(self, **kwargs):
        observations = super(RecordMultiCAM, self).reset(**kwargs)
        if not self.recording:
            self.start_video_recorder()
        return observations
    ...
    def step(self, action):
        time_step = super(RecordMultiCAM, self).step(action)
        observations, rewards, dones, infos = time_step
        ...
        if self.recording:
            self.video_recorder[0].append(
                (
                    get_multi_cam(
                        observations,
                        model=self.cam_model,
                        layer=[self.cam_model.actor_encoder.stacks[2].blocks[1].conv1]
                    ), copy.deepcopy(self.env.render(mode='rgb_array'))
                )
            )
            for i in range(self.video_num - 1):
                self.video_recorder[i + 1].append(
                    (
                        get_multi_cam(
                            observations,
                            model=self.cam_model,
    ...
    def close_video_recorder(self) -> None:
        if self.recorded_frames > 0:
            dump_multi_arr2video(self.video_num, self.video_recorder, self.video_folder)
        self.video_recorder = []
        for i in range(self.video_num):
            self.video_recorder.append([])
        self.recording = False

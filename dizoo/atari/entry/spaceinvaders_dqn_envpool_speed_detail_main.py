...
def sweep_main():
    ...
    if not good_pair:
        ...
        import time
        start_time = time.time()
        spaceinvaders_dqn_envpool_config.exp_name = f'SpaceInvaders-v5-envpool-speed-test-{wandb.config.collector_env_num}-{wandb.config.collector_batch_size}'
        spaceinvaders_dqn_envpool_config.env.collector_env_num=wandb.config.collector_env_num
        spaceinvaders_dqn_envpool_config.env.collector_batch_size=wandb.config.collector_batch_size
        main(EasyDict(spaceinvaders_dqn_envpool_config), max_iterations=5000000)
        print(time.time()-start_time)
        wandb.log({"time_cost": time.time()-start_time})
        #remove the directory named as exp_name
        shutil.rmtree(spaceinvaders_dqn_envpool_config.exp_name)
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(
        ...
        project='SpaceInvaders-v5-envpool-speed-test-detail'
        )
